#!/usr/bin/env python3
"""migrate-pattern-proposals-to-sql.py — STORE-P6-1f-C bulk migration.

Walks ``/vault/pattern_proposal/*.md`` and copies each markdown
pattern-proposal into the SQLite ``pattern_proposal`` table via the
ctrl-api endpoint shipped in STORE-P6-1f Round 1:

  * ``POST /api/v1/pattern-proposals`` (one row per file)
...
(raj313 → miguel → rapali → david — never in parallel):
  ...
  --phase move    Only safe after verify passes. Move source files to
                  ``/vault/_migrated_pattern_proposal/<date>/<original-relpath>``.
...
Per-tenant invocation:
  ...
  docker exec compose-alfred-learn-1 python3 \\
      /vault/_migrate/migrate-pattern-proposals-to-sql.py \\
...
PATTERN_PROPOSAL_DIR = "pattern_proposal"

FRONTMATTER_RE = re.compile(r"^---\n(.*?)\n---\n?(.*)$", re.DOTALL)

# UUIDv5 namespace — keeps ids deterministic across re-runs. The prefix
# is distinct from the audit/signal/observation prefixes so a path that
# happens to collide across record kinds (e.g. a basename reused for
# debugging) can never produce the same SQL row id.
NAMESPACE = uuid.NAMESPACE_DNS
NS_PREFIX = "vault-pattern-proposal:"
...
def parse_frontmatter(text: str) -> tuple[dict[str, Any], str]:
    ...
    Returns ``({}, text)`` when the file has no ``---`` fence — keeps
    the migration tolerant of half-formed records (P2-3 hit a few).
    """
...
def _opt_iso_to_ns(value: Any) -> int | None:
    """Coerce a frontmatter value into unix-ns. Returns None when the
    value is missing/empty/unparseable rather than raising.
    """
    if value is None:
        return None
    if isinstance(value, (int, float)):
        # Already numeric — assume ns when >= 10^15 (~ year 2001), else
        # seconds. Vault writers don't currently stamp these as numbers
        # but we want to round-trip cleanly if/when they do.
        v = float(value)
        if v >= 1e15:
            return int(v)
...
def _frontmatter_ts_ns(fm: dict[str, Any], src_path: Path) -> int:
    """Best-effort unix-ns timestamp for a pattern_proposal record.

    Priority:
      1. frontmatter.created (ISO 8601 — the canonical field the
         pattern_detection activity stamps)
      2. frontmatter.proposed_at / generated_at (alt names some
         writers used pre-Phase 6)
      3. filename ISO timestamp (yyyy-MM-ddTHH-mm-ssZ-<hash>.md)
      4. file mtime
      5. wall clock now (last-ditch)
    """
    for key in ("created", "proposed_at", "generated_at", "ts"):
        raw = fm.get(key)
...
def _member_observation_ids(fm: dict[str, Any]) -> list[str]:
    """Pull the cluster's observation refs out of frontmatter.

    The active writer in ``packages/learn/src/activities/pattern_detection.py``
    stamps ``observation_refs`` (vault paths). Older / parallel writers
    have used ``observation_ids`` (bare ids) and ``member_observations``
    (vault paths). We accept any of them and pass through verbatim so
    forensic reconstruction is possible.
    """
    for key in ("observation_refs", "observation_ids", "member_observations"):
        v = fm.get(key)
...
def build_post_body(
    ...
    proposed_name = _opt_str(fm, "name", "proposed_name", "label") or rel_path
    # cluster_size: prefer the explicit field, fall back to len(refs).
    raw_size = fm.get("cluster_size")
    if isinstance(raw_size, (int, float)):
        cluster_size = int(raw_size)
    else:
        cluster_size = len(_member_observation_ids(fm))

    member_ids = _member_observation_ids(fm)

    status = _opt_str(fm, "status") or "pending"
    reviewed_at_ns = _opt_iso_to_ns(fm.get("reviewed_at"))
    reviewed_by = _opt_str(fm, "reviewed_by")
    promoted_to = _opt_str(
        fm, "promoted_to_instinct_id", "promoted_to_instinct", "promoted_to"
    )

    ts_ns = _frontmatter_ts_ns(fm, src_path)

    # payload preserves the whole record so a round-trip can rebuild the
    # original markdown if needed. yaml safe_load can leave us with
    ...
    post: dict[str, Any] = {
        ...
        "proposed_body": body if body else "(empty)",
        "cluster_size": cluster_size,
        "member_observation_ids": json.dumps(member_ids, sort_keys=False),
        "status": status,
        "payload": json.dumps(payload, sort_keys=True, default=str),
    }
    if reviewed_at_ns is not None:
        post["reviewed_at"] = str(reviewed_at_ns)
    if reviewed_by is not None:
        post["reviewed_by"] = reviewed_by
    if promoted_to is not None:
        post["promoted_to_instinct_id"] = promoted_to
...
def phase_copy(args: argparse.Namespace) -> int:
    ...
    src_root = vault / PATTERN_PROPOSAL_DIR
    files = sorted(src_root.rglob("*.md")) if src_root.is_dir() else []
    print(
        f"[copy] {PATTERN_PROPOSAL_DIR}: {len(files)} files", flush=True
    )
    ...
    with manifest_path.open("a", encoding="utf-8") as manifest, err_path.open(
        ...
        for src in files:
            ...
            body, _fm = built
            try:
                r = client.post("/api/v1/pattern-proposals", json=body)
...
def phase_verify(args: argparse.Namespace) -> int:
    ...
    rng = random.Random(0x5706)  # deterministic — same sample on re-run
    ...
    for rel_path, row_id in sample:
        ...
        expected, _fm = built
        try:
            r = client.get(f"/api/v1/pattern-proposals/{row_id}")
        except httpx.HTTPError as e:
        ...
        if got.get("payload") != expected["payload"]:
            ...
            continue
        if got.get("proposed_body") != expected["proposed_body"]:
...
def phase_move(args: argparse.Namespace) -> int:
    ...
    if not manifest_path.exists():
        ...
        return 3
    date_tag = args.date
    dest_root = vault / "_migrated_pattern_proposal" / date_tag
...
def manifest_for(args: argparse.Namespace) -> Path:
    ...
    return (
        ...
        / f"migrate-pattern-proposals-{args.tenant}-{args.date}.log"
    )
