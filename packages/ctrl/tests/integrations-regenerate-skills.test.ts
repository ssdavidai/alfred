...
//
// 1. The handler reported `ok: true, actions: 50` for slack on David, but the
...
//    toolkit no longer had any ACTIVE connection.  David's slack file
...
// failure mode we hit on David's slack.
...
describe("POST /api/v1/integrations/regenerate-skills", () => {
  ...
  it("surfaces a real error (not ok: true) when writeFileSync silently drops the write", async () => {
    // This is the David-on-slack case in spirit: writeFileSync syscall returns
