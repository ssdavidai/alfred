...
//
// The bug (surfaced on David 2026-04-23): setupPlane() built its webhook URL
...
// `alfred-<slug>-<base36ts>`, matching what Hetzner + Cloudflare see). These
...
describe("generateSubdomain", () => {
  it("is a lowercase-only identity on SaaS-shaped customer names", () => {
    ...
    assert.equal(
      generateSubdomain("alfred-rapali-zsolt-mnczdq7a"),
      "alfred-rapali-zsolt-mnczdq7a",
    );
  });
...
describe("planeHostnameFromCustomerName", () => {
  it("preserves the full Hetzner slug in the webhook URL", () => {
  ...
  it("matches Cloudflare DNS for Miguel + Rapali", () => {
    assert.equal(
    ...
    assert.equal(
      planeHostnameFromCustomerName("alfred-rapali-zsolt-mnczdq7a").webhookUrl,
      "https://alfred-rapali-zsolt-mnczdq7a.alfred.black/api/v1/plane/webhook",
    );
  });

  it("regression: does NOT drop the Hetzner slug when customer_name includes it", () => {
    // This is the exact scenario that broke David's Plane webhook. Before
    ...
    assert.notEqual(
      ...
      "webhook URL must not drop the Hetzner slug",
    );
    assert.ok(
      r.webhookUrl.includes("mnbqn4jg"),
      "webhook URL must include the Hetzner slug",
    );
  });
