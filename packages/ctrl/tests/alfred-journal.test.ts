...
describe("appendJournal + resolvePrincipal", () => {
  ...
  it("bindPrincipalChannel is idempotent (re-bind same pair, no error)", () => {
    const db = makeDb();
    bindPrincipalChannel(db, "telegram", "432094090", "owner");
    bindPrincipalChannel(db, "telegram", "432094090", "owner"); // again
    assert.equal(resolvePrincipal(db, "telegram", "432094090"), "owner");
    db.close();
  });
...
describe("queryRecentJournal", () => {
  ...
  it("honours the limit bound", () => {
    ...
    const entries = queryRecentJournal(
      db,
      { channel: "telegram", chat_id: "432094090" },
      { limit: 5 },
    );
