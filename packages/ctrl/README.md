...
Fleet management tool for provisioning and managing [Alfred](../README.md) instances on Acme Cloud Cloud. Provides both an interactive TUI dashboard and a scriptable CLI.

## Prerequisites

- **Node.js 22+** (uses built-in `node:sqlite`)
- **ssh-keygen** (for Ed25519 keypair generation)
- A Acme Cloud Cloud API token (read/write)
...
├── infra/
│   ├── acme-cloud.ts         # Acme Cloud Cloud API client
...
│   ├── firewall.ts        # Acme Cloud firewall management
...
9. Configure restic backups to Acme Cloud Object Storage (if S3 credentials provided)
...
- **Firewall** — Acme Cloud cloud firewall + UFW: SSH and ICMP only inbound
...
|----------|----------|-------------|
| `HETZNER_API_TOKEN` | Yes | Acme Cloud Cloud API token (read/write) |
...
| `HETZNER_S3_ACCESS_KEY` | No | For restic backups to Acme Cloud Object Storage |
| `HETZNER_S3_SECRET_KEY` | No | For restic backups to Acme Cloud Object Storage |
...
| `DEFAULT_SERVER_TYPE` | No | Default Acme Cloud server type (default: cx33) |
| `DEFAULT_LOCATION` | No | Default Acme Cloud location (default: fsn1) |
