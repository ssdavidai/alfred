...
// STORE-P2-5: the markdown-walking audit helpers (parseSimpleYaml,
// classifyAuditKind, isAutomatedAuditKind, readNeedsAttentionHeadline,
// formatAuditSummary, sentenceCase, toSentenceVerb, quote) were deleted
// along with their only caller, GET /api/v1/admin/audit. The SQL
// /api/v1/audit endpoint (audit.ts) is now the sole audit read path.
...
export function registerAdminRoutes(): void {
  ...
  // STORE-P2-5: the legacy markdown-walking GET /api/v1/admin/audit was
  // retired here. It read vault/event/*.md records and rendered a
  // best-effort summary line per file; STORE-P2-2 + P2-3 moved every
  // audit writer onto the SQL `audit` table and bulk-migrated the
  // historical markdown trail. The SaaS reads /api/v1/audit (audit.ts)
  // exclusively now. Source files moved to /vault/_migrated_audit/ on
  // every tenant before P2-5 deleted them.
