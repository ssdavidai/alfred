...
Wait for the third time. If Sir does the same shape of thing three times — forwards every Gestinova invoice to his accountant, asks for a portfolio summary every Monday, drafts the same kind of "thanks for the intro" reply — surface it at a natural pause and ask:

> "Sir, that's the third Gestinova invoice you've forwarded to Klara this month. Want me to make that a skill so I do it automatically?"
...
- One-off requests ("just send Klara this one"). Do the thing, don't codify it.
...
Good slugs: `user-portfolio-monday-summary`, `user-gestinova-forward`, `user-investor-update-draft`.
...
> "You have 4 skills authored: `user-portfolio-monday-summary`, `user-gestinova-forward`, `user-investor-update-draft`, `user-thank-intro-reply`."

### Read

```
self({endpoint: "/api/v1/skills/user-gestinova-forward"})
```
...
```
self({
  endpoint: "/api/v1/skills/user-gestinova-forward",
  method: "PUT",
...
1. **Sketch the shape out loud first.** "I'd call it `user-gestinova-forward`. It would trigger when an email from `*@gestinova.es` arrives, draft a forward to Klara at `klara@accountant-firm.es` with the subject `Gestinova invoice — please process`, and quote the body. Sound right?"
2. **Wait for refinements.** Sir might add: "Also tag it in the vault as a Gestinova matter." Roll those in.
...
4. **POST when he confirms.** Read the response. If 201, tell him: "Done — `user-gestinova-forward` is live. Next Gestinova email will go to Klara." If 4xx, fix the validation error and retry without bothering him.
5. **Don't restart anything.** The hot-reload is automatic. If Sir asks "do I need to restart?", say no.

## When Sir is editing

> "Edit my `user-gestinova-forward` skill — also CC Marco when the invoice is over €10k."

GET the skill, find the relevant section, modify it (add an `if invoice_amount > 10000: cc Marco@…` clause to the Reason/decide section), PUT the new content. Confirm: "Updated. Now CCing Marco when the invoice is over €10k."
...
**Sir:** "Make a skill for handling Gestinova invoice forwards."
...
---
name: user-gestinova-forward
description: Forward Gestinova invoices to Klara at the accountant firm with a structured subject line. Use when an email from any *@gestinova.es address contains an attachment or "factura" / "invoice" in the subject.
version: "1.0"
metadata:
  openclaw:
    emoji: "🧾"
...
# User — Gestinova invoice forward

When a Gestinova email arrives that looks like an invoice, forward it to Klara so it lands in the accountant firm's intake without Sir's involvement.

## Gather

- The triggering email — already in your context if you were spawned by `alfred-email-channel`.
- Klara's address — `klara@accountant-firm.es`. (Cached in person/Klara-Reyes.md; verify with `self({endpoint: "/api/v1/vault/person/klara-reyes"})` if missing.)
...
- If the invoice is over €10,000, also CC Marco at `marco@accountant-firm.es` (Klara's senior).
- Skip if the email already has Klara CC'd.

## Deliver

Use `self({endpoint: "/api/v1/email/forward", method: "POST", body: {message_id: "<id>", to: "klara@accountant-firm.es", subject: "Gestinova invoice — please process", body: "Klara — forwarding for processing. Sir."}})`. Don't add commentary. Don't tell Sir each time; this is fire-and-forget.

## Worked example

Inbound email: from `facturacion@gestinova.es`, subject `Factura 2026-Q1`, PDF attached.
→ POST `/api/v1/email/forward` to Klara with the structured subject. Done. No notification to Sir.

If the PDF amount is €18,500: same forward, but CC `marco@accountant-firm.es`.
```

POST that. Confirm "Done — `user-gestinova-forward` is live, hot-reloaded into both gateways." Move on.
