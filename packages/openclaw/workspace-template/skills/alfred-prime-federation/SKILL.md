...
This skill is **only present on Alfred Prime** (David's instance). Its tools (`tenant`, `ask_alfred`) are only registered when the server is started with `ALFRED_PRIME=true` and a populated `CROSS_TENANT_PEERS` env var.
...
### Use `tenant` when…
- Sir asks a direct factual question about a peer's vault ("how many active matters does Miguel have?")
...
- The question requires the peer's Alfred to reason over his own vault in natural language
  - "Ask Miguel what his top priority is this week"
  - "Have Rapali summarize his meetings with Katalin from April"
...
{
  "peers": [
    {"id": "miguel",   "label": "Miguel Ortiz (Upstring)"},
    {"id": "rapali",   "label": "Zsolt Rapali"},
...
### Direct read
**Sir: "How many unprocessed events are in Miguel's inbox right now?"**
```
tenant({ tenant: "miguel", endpoint: "/api/v1/streams/events", query: { status: "unprocessed" } })
```

### Direct write
**Sir: "Create a note on Miguel's vault about the Gestinova workshop prep."**
```
tenant({
  tenant: "miguel",
  ...
  body: {
    type: "note",
    name: "gestinova-workshop-prep",
    content: "---\ntype: note\nname: gestinova-workshop-prep\nstatus: active\ncreated: 2026-04-19\n---\n\n# Gestinova workshop prep\n\n..."
  }
})
```

### Delegated reasoning
**Sir: "Ask Miguel's Alfred what should get Miguel's attention first thing Monday."**
```
ask_alfred({
  tenant: "miguel",
...
### Multi-tenant sweep
**Sir: "Which tenants have a chore that references Retool?"**
```
// Loop in your head over peers returned by /cross-tenant/peers:
for peerId of ["miguel", "rapali", "daveszab"] {
  tenant({ tenant: peerId, endpoint: "/api/v1/vault/search", query: { grep: "Retool", type: "chore" } })
}
...
1. **Never use `tenant` to write without Sir's explicit approval for THAT specific write.** Cross-tenant writes are high-stakes — mistakes land in the wrong vault and are hard to untangle (we learned this from the Gestinova99 incident). Read-before-write applies double here.
