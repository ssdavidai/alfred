...
The audit is the after-the-fact tripwire for the April 2026 Rapali-vs-
David Composio user_id leak. These tests cover:
...
class TestAuditStreamsActivity:
    ...
    async def test_detects_mismatch_in_composio_file(
        ...
        calendar.write_text("\n".join([
            ...
            json.dumps({
                "source_ref": "evt-leaked-from-rapali",
                "received_at": "2026-04-15T10:00:00Z",
                "raw": {
                    "attendees": [
                        {"email": "rapali@example.com", "self": True},
                    ],
                    "start": {"dateTime": "2026-04-15T10:00:00Z"},
                },
            }),
        ...
        assert entries[0]["event_id"] == "evt-leaked-from-rapali"
        assert entries[0]["self_email"] == "rapali@example.com"
