...
def test_clean_name_collapses_whitespace():
    assert clean_name("Wolt   \t  Hungary") == "wolt hungary"
...
def test_proposals_sorted_by_txn_count_desc():
    txns = (
        [_txn(id=f"a{i}", name="Wolt") for i in range(10)]
...
def test_dominant_currency_and_account():
    txns = [
        _txn(id="1", name="Wolt", currency="HUF", account_name="CIB HUF"),
        _txn(id="2", name="Wolt", currency="HUF", account_name="CIB HUF"),
        _txn(id="3", name="Wolt", currency="EUR", account_name="Revolut EUR"),
    ]
    props = cluster_transactions(txns, min_group_size=2)
    assert len(props) == 1
    assert props[0].dominant_currency == "HUF"
    assert props[0].dominant_account == "CIB HUF"
