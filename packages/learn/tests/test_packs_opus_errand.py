...
def _sample_onboard() -> dict:
    return {
        "facts": [
            {"category": "professional", "fact": "Stan sent contract revision Tuesday", "confidence": "high"},
            {"category": "personal", "fact": "Hanna's 9-month checkup is due", "confidence": "high"},
        ...
        "matters_brief": [{"name": "Stan collaboration"}, {"name": "Hanna logistics"}],
...
_OPUS_GOOD_ERRAND_RESPONSE = json.dumps({
    "errands": [
        {
            "name": "Reply to Stan's contract revision",
            "status": "todo",
            "owner": "human",
            "urgency": "high",
            "due_hint": "by Friday",
            "related_matter": "Stan collaboration",
            "context": (
                "Stan sent over a revised contract on Tuesday with new payment terms. "
            ...
            "dependencies": ["Read the contract", "Confirm Stylers cash flow with Eszter"],
...
class TestBuildRichErrandContent:
    def _good(self):
        return {
            ...
            "dependencies": ["Confirm with Eszter"],
        }
...
class TestBuildErrandPrompt:
    ...
    def test_prompt_includes_matters(self):
        prompt = _build_errand_prompt([], [], [], {}, "", [{"name": "Stan project"}])
        assert "Stan project" in prompt

    def test_prompt_includes_facts(self):
        facts = [{"category": "personal", "fact": "Hanna pickup tomorrow"}]
        prompt = _build_errand_prompt(facts, [], [], {}, "", [])
        assert "Hanna pickup tomorrow" in prompt
