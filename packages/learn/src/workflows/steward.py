"""StewardWorkflow — per-matter perception-and-action loop (#835 Phase 0).

Each matter (``matter/*.md`` in the vault) gets a Temporal Schedule
named ``al-steward-<matter-slug>`` that fires this workflow every 30
minutes (Layer 3 default cadence — see RFC #832 §3). The workflow
iterates the matter's tasks, evaluates the ones whose
``next_check_after`` has elapsed, and stamps ``last_steward_check_at``
on each evaluated task.

Phase 0 is scaffold-only: ``evaluate_task`` is a no-op. Phase 1 (#837)
swaps in the real shadow LLM evaluation without changing this
workflow's signature.

Yield budget
------------
All vault I/O lives in activities. The workflow holds at most a list
of task descriptors in memory; on a typical matter that's tens of
...
class StewardResult:
    ...
    the caller (mostly tests; cron callers ignore it)."""
    matter_id: str = ""
    evaluated: int = 0
    skipped: int = 0
    errors: int = 0
...
class StewardWorkflow:
    """Per-matter Steward tick. Schedule: every 30 min per matter."""
    ...
    async def run(self, matter_id: str) -> StewardResult:
        ...
        # 1. Load the matter's tasks. List endpoint walks the whole
        #    vault but the response is cheap (preview=0). On a tenant
        #    with thousands of tasks across many matters this is the
        #    one expensive call per tick — tolerable at 30-min cadence.
        try:
        ...
        #    next_check_after is still in the future. Everything else
        #    gets the no-op evaluation + the cursor stamp.
        #
        #    Phase 2 (#838) tracks the matter-aggregate signal arrival
        #    so the post-loop ``update_matter_cadence`` can advance the
        #    backoff counter. Any single task in this matter producing
        #    signals counts as "the matter saw a signal this tick".
        ...
        for idx, task in enumerate(tasks):
            ...
            if not task_id:
                # Defensive — load_matter_tasks never returns these,
                # but the loop tolerates them so a malformed entry
                # can't crash the whole tick.
                result.skipped += 1
                continue
            ...
            # 3. Evaluate. Phase 1 promoted this from a no-op to a real
            #    LLM call; Phase 2 widens the timeout (gated on
            #    workflow.patched() to preserve replay determinism for
            #    workflows started under Phase 1) because the new
            #    Sure / Gmail / ctrl-api stream gatherers can each take
            #    a few seconds before the LLM call even runs.
            ...
            try:
                ...
                # Counts as evaluated (the no-op succeeded) but we
                # surface the cursor-write failure so david-side
                # observability shows it.
                result.evaluated += 1
                continue

            # Phase 2 (#838) — collect matter-aggregate signal stats from
            # the per-task outcome so the matter-cadence backoff after
            # the loop has accurate input.
        ...
        if workflow.patched("steward-phase2-matter-cadence"):
            try:
                ...
                # Non-fatal: cadence state is observability-only in
                # Phase 2 (Phase 5 wires the actual schedule update).
                workflow.logger.warning(
