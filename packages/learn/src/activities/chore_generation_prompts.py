...
DO NOT emit this (tenant-c incident, 2026-04-29):
...
**Event-driven chores.** If the opportunity is "react to signals" rather than "run on a calendar" (e.g. "watch Acme Tools emails"), pick a daily or twice-daily polling cadence — e.g. `0 7 * * *` (daily at 7am UTC) for a morning sweep. Never emit `* * * * *` or sub-minute crons — you will be rate-limited and flood the user.
