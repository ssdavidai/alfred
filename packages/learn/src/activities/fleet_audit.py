...
In April 2026 we discovered that ~8 days of Rapali's Gmail/Calendar events
had been ingested under David's tenant due to a misconfigured Composio
user_id. The leak went unnoticed because the streams look like ordinary
Google events — there's no surface-level difference between "David's own
calendar event" and "an event from Rapali's account that happened to land
on David's stream JSONL".
...
the exact signature of the Rapali-vs-David leak.
