...
# Hermes web dashboard — exposes the main profile's config / keys / sessions
# UI on port 9119. Bound to 0.0.0.0 so Caddy (on the same docker network)
# can reverse-proxy hermes.{$DOMAIN} → hermes:9119. The `--insecure` flag is
# Hermes' own check — its warning is about exposing the dashboard on a
# public network, which is mitigated here by Caddy's basic_auth (creds
# generated by init, stored in .env + /alfred-data/.hermes-dashboard-password).
# `--port 9119` is the canonical default; pinned explicit in case a future
# default shifts.
#
# NOTE on --skip-build: the hermes image does NOT bake the dashboard's web
# dist (hermes_cli/web_dist/ is empty in the upstream wheel). First boot
# runs `npm install && npm run build` automatically (~1-2 min) and persists
# the dist for subsequent boots. Don't pass --skip-build until the upstream
# image starts shipping pre-built assets, or we bake them ourselves.
start_proc "hermes-dashboard" "cd \"${PROFILES_DIR}/main\" && exec hermes -p main dashboard --host 0.0.0.0 --port 9119 --insecure --no-open"
