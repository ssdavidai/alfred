...
# `--skip-build` avoids the npm-build step at startup (the image ships the
# pre-built dist; rebuilding adds ~1min to boot for no behavioral gain).
# `--port 9119` is the canonical default; pinned explicit in case a future
# default shifts.
start_proc "hermes-dashboard" "cd \"${PROFILES_DIR}/main\" && exec hermes -p main dashboard --host 0.0.0.0 --port 9119 --insecure --no-open --skip-build"
