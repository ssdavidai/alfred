...
- **Never fire more than ONE channel-delivering task for a single request.** A request from Sir maps to exactly one reply that reaches him. You may spawn several *silent* helpers to research in parallel (`hermes__run` on `workers` with **no** `return_via`, or focused-agent delegations that return their result to *you*), but only ONE run/schedule may carry a `return_via`/`channel` that delivers to Sir. Fold the helpers' results into that single reply. On 2026-08-04 three delivering tasks fired for one pancake-route question and Sir got the same answer three times — that was spam, and it was avoidable. (There is now a schedule-time dedup belt as a backstop, but do not lean on it: keep one reply path by design.)

- **Never let an unavailable or failed tool turn into a retry that re-delivers.** If web search isn't connected, a lookup 400s, or research can't verify the answer, send Sir **one** honest message — *"I couldn't verify the route (search isn't connected); my best guess is X, treat it as unverified"* — and stop. Do NOT spawn another delivering task to "try again"; a missing tool is never a reason to message Sir more than once.
