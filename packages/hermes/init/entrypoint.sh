...
# ── 10. Hermes web dashboard basic-auth credentials ─────────────────────────
# The hermes container's supervisor.sh launches `hermes dashboard --host
# 0.0.0.0 --insecure` as a 4th supervised process (port 9119). Caddy proxies
# hermes.{$DOMAIN} → hermes:9119 and gates it behind HTTP basic-auth using
# the env vars below.
#
# Idempotent: re-running init preserves existing credentials. Once Sir has
# bookmarked the password, never regenerate it without explicit operator
# action. The raw password lives at /alfred-data/.hermes-dashboard-password
# (chmod 600 — the operator retrieves it once via ssh+cat, then bookmarks).
HERMES_DASHBOARD_PW_FILE=/alfred-data/.hermes-dashboard-password
HERMES_DASHBOARD_ENV_FILE=/alfred-data/.hermes-dashboard.env

if [[ ! -f "$HERMES_DASHBOARD_PW_FILE" || ! -s "$HERMES_DASHBOARD_PW_FILE" ]]; then
    # First boot: generate a 28-char URL-safe random password, then bcrypt
    # it with cost 14 (matches Caddy's recommended default for HTTP basic-auth).
    HERMES_DASHBOARD_USER="${HERMES_DASHBOARD_USER:-admin}"
    HERMES_DASHBOARD_PW=$(python3 -c "import secrets,string; print(''.join(secrets.choice(string.ascii_letters+string.digits+'-_') for _ in range(28)))")
    HERMES_DASHBOARD_BCRYPT=$(python3 -c "
import bcrypt
pw='$HERMES_DASHBOARD_PW'.encode()
print(bcrypt.hashpw(pw, bcrypt.gensalt(rounds=14)).decode())
" 2>/dev/null)

    if [[ -z "$HERMES_DASHBOARD_BCRYPT" ]]; then
        echo "[init] WARNING: bcrypt module unavailable in init image — falling back to htpasswd"
        # htpasswd is in apache2-utils which the init image may not have.
        # If neither works, log + skip; Caddy basic-auth will fail-open with
        # an empty hash directive (fast failure with a clear error in caddy logs).
        if command -v htpasswd >/dev/null 2>&1; then
            HERMES_DASHBOARD_BCRYPT=$(htpasswd -nbBC 14 "$HERMES_DASHBOARD_USER" "$HERMES_DASHBOARD_PW" 2>/dev/null | cut -d: -f2-)
        fi
    fi

    if [[ -n "$HERMES_DASHBOARD_BCRYPT" ]]; then
        printf '%s' "$HERMES_DASHBOARD_PW" > "$HERMES_DASHBOARD_PW_FILE"
        chmod 600 "$HERMES_DASHBOARD_PW_FILE" 2>/dev/null || true
        printf 'HERMES_DASHBOARD_USER=%s\nHERMES_DASHBOARD_BCRYPT=%s\n' \
            "$HERMES_DASHBOARD_USER" "$HERMES_DASHBOARD_BCRYPT" \
            > "$HERMES_DASHBOARD_ENV_FILE"
        chmod 600 "$HERMES_DASHBOARD_ENV_FILE" 2>/dev/null || true
        echo "[init] Generated Hermes dashboard credentials (user=$HERMES_DASHBOARD_USER)"
        echo "[init]   Raw password: $HERMES_DASHBOARD_PW_FILE"
        echo "[init]   Caddy env:    $HERMES_DASHBOARD_ENV_FILE"
        echo "[init]   IMPORTANT: copy HERMES_DASHBOARD_USER + HERMES_DASHBOARD_BCRYPT into /opt/alfred/.env so the caddy service picks them up at next restart."
    else
        echo "[init] WARNING: could not generate Hermes dashboard bcrypt — install python3-bcrypt or apache2-utils in the init image."
    fi
else
    echo "[init] Hermes dashboard password already present, reusing ($HERMES_DASHBOARD_PW_FILE)"
fi
