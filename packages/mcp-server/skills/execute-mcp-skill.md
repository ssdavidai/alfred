...
2. **Composio's `user_id` is the tenant identity, not Sir's third-party account.** When responses include `user_id: "alfred-david-mnbqn4jg-1"` etc., that's Composio's per-tenant scoping — never feed it back into the third-party API as a user identifier (Slack U-IDs, Gmail addresses, GitHub usernames). Pull the real per-app identity from KNOWN_CONTACTS via the `alfred` connector's vault tools.
