...
test("derive: ready — full-seed headless redeem completed (PAPERCLIP_AGENT_TOKEN seeded)", () => {
  // The full-seed PR (2026-05-27): bootstrap-paperclip.sh steps 6–11 ran,
  // PAPERCLIP_AGENT_TOKEN is in /opt/alfred/.env, ctrl-api surfaces the
  // ready state + seed_company_name + seed_agent_name. The principal NEVER
  // sees the wizard — the card shows "Open Paperclip →" + the inline
  // "Alfred · hermes (CEO)" label.
  const card = derivePaperclipCardState(
    {
      ...BASE,
      has_signing_secret: true,
      setup_state: "ready",
      seed_company_name: "Alfred",
      seed_company_id: "449a5cf9-adfe-4d53-9aac-625e5e1104c2",
      seed_agent_name: "hermes",
      seed_agent_id: "694921cb-a3c5-46a1-8472-04a31052df95",
    },
    FROZEN_NOW,
  );
  assert.equal(card.status, "ready");
  assert.equal(card.pillLabel, "Ready");
  assert.equal(card.pillTone, "active");
  // Test affordance enabled the moment the seed lands — the heartbeat
  // path is wired (PAPERCLIP_HEARTBEAT_SECRET is auto-generated by
  // bootstrap.sh, so has_signing_secret is true here).
  assert.equal(card.canTest, true);
  // Inline label: company + agent + role.
  assert.equal(card.seedLabel, "Alfred · hermes (CEO)");
  // Deep-link origin still derivable from heartbeat_url.
  assert.equal(card.paperclipOrigin, "https://paperclip.home.alfred.black");
  // Admin invite URL must NOT be surfaced on ready (it would be stale).
  assert.equal(card.adminInviteUrl, "");
});

test("derive: ready — falls back to default names when seed metadata is missing", () => {
  // ctrl-api may serve a `ready` state without the seed_* fields if the
  // /alfred-data/paperclip-seed-credentials.json file was deleted and no
  // env fallback is set. The card should still render with sensible
  // defaults so the principal sees a working button rather than "undefined".
  const card = derivePaperclipCardState(
    {
      ...BASE,
      has_signing_secret: true,
      setup_state: "ready",
      // seed_* fields omitted
    },
    FROZEN_NOW,
  );
  assert.equal(card.status, "ready");
  // Defaults: company "Alfred", agent "hermes".
  assert.equal(card.seedLabel, "Alfred · hermes (CEO)");
});
