...
test("buildInboxUsername strips and normalises common email patterns", () => {
  assert.equal(buildInboxUsername("owner@example.com"), "alfred.owner");
  assert.equal(buildInboxUsername("owner2@example.com"), "alfred.owner2");
  assert.equal(
...
test("buildFallbackUsername yields a stable prefix + 3-digit suffix", () => {
  const fb = buildFallbackUsername("owner@example.com");
  assert.match(fb, /^alfred\.owner\d{3}$/);
});
