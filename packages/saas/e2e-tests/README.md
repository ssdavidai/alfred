...
3. Logs in (email verification skipped via env var)
4. Sees pricing page
...
# 2. Start the app with email verification skipped (in a separate terminal)
cd app && SKIP_EMAIL_VERIFICATION_IN_DEV=true wasp start

# 3. Install test dependencies and run the registration flow test
cd e2e-tests && npm install
npm run test:e2e
```

Or with the Playwright UI for debugging:
