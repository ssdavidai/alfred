...
> = async (rawPaymentPlanId, context) => {
  ...
  // Enforce closed beta gate server-side
  const closedBeta = await context.entities.AdminSetting.findUnique({
    where: { key: "closed_beta" },
  });
  if (closedBeta?.value === "true") {
    throw new HttpError(403, "Alfred is currently in closed beta. Join the waitlist for early access.");
  }
  ...
  if (!paymentPlan) {
    throw new HttpError(400, `Plan "${paymentPlanId}" is not available for self-serve checkout`);
  }
